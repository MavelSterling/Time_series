...
ur.kpss <- function(y, type=c("mu", "tau"), lags=c("short", "long", "nil"), use.lag=NULL){
  y <- na.omit(as.vector(y)) # omite valores na
  n <- length(y) # tamaño del vector
  type <- match.arg(type)
  lags <- match.arg(lags)
  if(!(is.null(use.lag))){
...
summary(ur.kpss(diff(lz), use.lag = 0, type = "tau" )) # intercepto y tendencia
summary(ur.kpss(diff(lz), use.lag = 0, type = "mu" ) )# intercepto
